describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems_shadow = await prodItems.getProperty('shadowRoot')
    const button = await prodItems_shadow.$('button')
    ...
    const expectedText = 'Remove from Cart'

    expect(text['_remoteObject'].value).toBe(expectedText);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1;i < prodItems.length;i++) {
      const prodItems_shadow = await prodItems[i].getProperty('shadowRoot')
      const button = await prodItems_shadow.$('button');
      await button.click()
    }
    // Check to see if the innerText of #cart-count is 20
    var cc = await page.$('#cart-count')
    var count = await cc.getProperty('innerText')
